...
class _ListTaskScreen extends State<ListTaskScreen> {
            ...
            return SimpleDialog(
              ...
              children: [
                ...
                Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                  Text("Status: "),
                  IconButton(
                      icon: getSortIcon(_sortByCategory),
                      onPressed: () {
                        setState(() {
                          _sortByCategory++;
  ...
  List<Task> getFilteredTask() {
    var ans =
        _tasks.where((element) => element.title.contains(_search)).toList();
    ans.sort((task1, task2) {
      if (_sortByDue == 1) {
        return task1.due.compareTo(task2.due);
      } else if (_sortByDue == 2) {
        return task2.due.compareTo(task1.due);
      }
      return 0;
    });
    ans.sort((task1, task2) {
